...
function renderTrainerRunningSamples() {
  ...
  // Per-step galleries, newest checkpoint first. We deliberately do NOT
  // build a cross-step prompt×step matrix any more: that relied on matching
  // a parsed prompt-slug string across steps, and whenever the AI Toolkit
  // filename format drifted the slug would absorb the step number — making
  // every step look like a unique prompt, which rendered as a sparse
  // diagonal with a wall of empty `—` placeholder cells. A simple gallery
  // grouped by step can never produce a phantom empty cell, and images keep
  // their natural aspect ratio (no portrait cropping).
  const checkpoints = [...(data.checkpoints || [])]
    .sort((a, b) => Number(b.step) - Number(a.step));
  const groups = checkpoints.map(c => {
    const samples = c.samples || [];
    const thumbs = samples.map(s => `
      <figure class="sample-thumb">
        <img src="${esc(s.url)}" alt="${esc(s.name)}" loading="lazy" decoding="async">
        ${s.prompt_slug ? `<figcaption title="${esc(s.prompt_slug)}">${esc(s.prompt_slug)}</figcaption>` : ''}
      </figure>`).join('');
    return `
      <section class="sample-step-group">
        <div class="sample-step-label">
          <span>step ${Number(c.step).toLocaleString()}</span>
          <span class="sample-step-count">${samples.length} ${samples.length === 1 ? 'image' : 'images'}</span>
        </div>
        <div class="sample-thumbs">${thumbs || '<div class="px-mute" style="font-size:11px">no images</div>'}</div>
      </section>`;
  }).join('');
  body.innerHTML = `<div class="sample-timeline">${groups}</div>`;
}
