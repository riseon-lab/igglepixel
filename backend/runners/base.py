...
def _data_key() -> bytes:
    """Read the at-rest data key from the env. Raises RuntimeError if missing."""
    hex_key = os.environ.get("FORGE_DATA_KEY")
    if not hex_key:
        raise RuntimeError("CRITICAL: FORGE_DATA_KEY environment variable is not set. At-rest encryption key is mandatory.")
...
class Runner(ABC):
    ...
    def save_bytes(data: bytes, dest: Path) -> Path:
        """Save arbitrary generated bytes — encrypted with FORGE_DATA_KEY."""
        key = _data_key()
        import backend.crypto as fcrypto
        return fcrypto.write_encrypted(key, dest, data)
    ...
    def save_image(image, dest: Path, format: str = "PNG") -> Path:
        """Save a PIL image — encrypted with FORGE_DATA_KEY.

        Returns the on-disk path (with `.enc` suffix).
        """
        buf = io.BytesIO()
